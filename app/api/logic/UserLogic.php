...
class UserLogic extends BaseLogic
{
    ...
    public static function info(int $userId)
    {
        $user = User::where(['id' => $userId])
            ->field('id,sn,sex,account,password,nickname,real_name,avatar,mobile,create_time,user_money,points,icode')
            ->findOrEmpty();
        $user['has_password'] = !empty($user['password']);
        $user['has_auth'] = 0;
    ...
    public static function changePassword(array $params, int $userId)
    {
        try {
            ...
            // 保存密码
            $user->password = LoginLogic::getPwdEncryptString($params['new_password']);;
            $user->save();
